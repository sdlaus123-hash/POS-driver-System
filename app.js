  ...
  async function initAttendance() {
    ...
    const setDoneMessage = (action, finished) => {
      ...
      if (action === "start" && !finished) {
        ...
        setText("doneAttendanceText", `出勤時刻 ${timeText(state.startTime)} / 今日もよろしくお願いします`, "保存しました");
        return;
      }
      title.textContent = action === "end" ? "退勤を保存しました" : "本日の勤務は完了しています";
      setText("doneAttendanceText", `出勤 ${timeText(state.startTime)} / 退勤 ${timeText(state.endTime)} / お疲れ様でした。前払い申請対象として反映されます`, "保存しました");
    };
