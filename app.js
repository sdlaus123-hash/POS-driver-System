(function () {
  ...
  const DEMO_SEED_VERSION = "poms-demo-advance-preview-2026-05-14";
  ...
  function renderDriver(driver) {
    ...
    if ($("advanceUnitPriceLabel")) $("advanceUnitPriceLabel").textContent = formatMoney(driver.unitPrice || 0);
  ...
  async function initAdvance() {
    ...
    async function calculateAdvance() {
      ...
      if (config.API_BASE_URL && dateFrom && dateTo) {
        try {
          const remote = await apiGetDriver({ type: "advance_calc", driverId: driver.id, dateFrom, dateTo, selectedDates: selectedAvailableDates.join(",") });
