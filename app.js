(function () {
  ...
  function seedDemoData(force = false) {
    ...
    const makeAdvance = (driverId, fromDay, toDay) => {
      ...
      const workedDays = attendanceRows.filter((row) => row.driverId === driverId && row.date >= dateFrom && row.date <= dateTo && !isOffStatus(row.status)).length;
  ...
  async function initAttendance() {
    ...
    function render() {
      if (!attendanceResolved) {
        $("attendanceStatus").textContent = "確認中";
        ...
        $("attendanceLiveCard").classList.remove("working", "finished");
        $("attendanceActionTitle").textContent = "勤務状況を確認しています";
        $("attendanceActionText").textContent = "すぐに表示されます。少しだけお待ちください。";
        return;
      }
  ...
  async function initAdvance() {
    ...
    function localWorkedDatesForMonth(month) {
      return new Set(readStore("attendance", [])
        ...
        .filter((row) => row.driverId === driver.id && inMonth(row.date, month) && !isOffStatus(row.status))
        .map((row) => row.date));
    }
    ...
    function isDateApplied(date) {
      return calendarAppliedDates.has(date) || currentDriverAdvances().some((row) => rangesOverlap(date, date, row.dateFrom || row.date, row.dateTo || row.date));
    }
    ...
    function findOverlap(dateFrom, dateTo) {
      ...
      const targetDates = selectedAvailableDates.length
        ? selectedAvailableDates
        : Array.from(calendarWorkedDates).filter((date) => date >= start && date <= end);
      return currentDriverAdvances().find((row) => {
        if (!rangesOverlap(dateFrom, dateTo, row.dateFrom || row.date, row.dateTo || row.date)) return false;
        return targetDates.some((date) => rangesOverlap(date, date, row.dateFrom || row.date, row.dateTo || row.date));
      });
    }
    ...
    function availableDatesLocal(dateFrom, dateTo) {
      ...
      readStore("attendance", [])
        .map(normalizeAttendanceRow)
        .filter((row) => row.driverId === driver.id && row.date >= start && row.date <= end && !isOffStatus(row.status))
        .forEach((row) => {
          if (!isDateApplied(row.date)) dates.add(row.date);
        });
    ...
    function renderSelectedDates() {
      ...
      if (!box || !count) return;
      count.textContent = `${selectedAvailableDates.length}日分`;
    ...
    async function calculateAdvance() {
      ...
      const dateTo = $("advanceDateTo").value;
      const seq = ++calcSeq;
      selectedAvailableDates = availableDatesLocal(dateFrom, dateTo);
      let workedDays = selectedAvailableDates.length;
      if (config.API_BASE_URL && dateFrom && dateTo) {
        try {
          const remote = await apiGetDriver({ type: "advance_calc", driverId: driver.id, dateFrom, dateTo });
          if (seq !== calcSeq) return;
          if (remote && remote.ok) {
            selectedAvailableDates = Array.isArray(remote.availableDates)
              ? remote.availableDates
              : Array.isArray(remote.selectedAvailableDates)
                ? remote.selectedAvailableDates
                : Array.isArray(remote.workedDates)
                  ? remote.workedDates
                  : selectedAvailableDates;
            workedDays = Number(remote.workedDays || selectedAvailableDates.length || 0);
            if (Array.isArray(remote.advances)) {
              const allAdvances = readStore("advance", []);
              const others = allAdvances.filter((row) => row.driverId !== driver.id);
              writeStore("advance", [...others, ...remote.advances.map(normalizeAdvanceRow)]);
    ...
    function updateSummary() {
      ...
      const amount = Number($("advanceAmount").value || 0);
      const salesAmount = Number(driver.unitPrice || 0) * workedDays;
      const percentFee = amount ? Math.round(amount * 0.08) : 0;
    ...
    async function submit() {
      ...
      const payload = {
        ...
        transferAmount,
        tag: "通常",
        note: selectedAvailableDates.length ? `稼働日: ${selectedAvailableDates.map(formatShortDate).join("、")}` : "",
        bankName: driver.bankName || "",
        branchName: driver.branchName || "",
        accountNumber: driver.accountNumber || "",
        accountHolder: driver.accountHolder || "",
  ...
  function driverMonthlyStats(data, driver) {
    ...
    rows.forEach((row) => {
      if (isOffStatus(row.status)) return;
