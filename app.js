(function () {
  ...
  async function fetchLiveLineProfile_() {
    ...
    return { ...normalized, lineAccessToken: window.liff.getAccessToken ? (window.liff.getAccessToken() || "") : "" };
  }
  ...
  async function tryDriverAutoLoginByLine(requestedDriverId = "") {
    ...
    const profile = await getLineProfile({ preferCached: false, refreshInBackground: false });
    if (!profile || !profile.userId) return null;
    const result = await apiPost({
      type: "driver_line_auto_login",
      lineUserId: profile.userId,
      lineAccessToken: profile.lineAccessToken || "",
  ...
  function renderDriver(driver) {
    ...
    if (site) site.textContent = page === "advance" ? siteName : `${siteName} ・ ${contractType}`;
  ...
  async function initAdvance() {
    ...
    function renderAdvanceCalendar() {
      ...
      if (availableBox) {
        ...
        for (let day = 1; day <= lastDay; day += 1) {
          ...
          button.setAttribute("aria-pressed", selected && !applied ? "true" : "false");
          button.innerHTML = `<strong>${formatShortDate(date)}</strong><span>${applied ? "申請済み" : selected ? "選択中" : dayOfWeekLabel(date) + "曜"}</span>`;
          if (applied) {
        ...
        if ($("advanceQuickAvailable")) $("advanceQuickAvailable").textContent = availableCount ? `${availableCount}日` : "0日";
        return;
      }
    ...
    function renderOverlap(dateFrom, dateTo) {
      ...
      if (!selectedAvailableDates.length) {
        if (text) text.classList.add("advance-preview-hidden");
        text.textContent = "申請できる稼働日を選択してください";
        syncSubmitReady();
        return;
      }
      if (text) text.classList.remove("advance-preview-hidden");
      if (overlap) {
    ...
    function renderSelectedDates() {
      ...
      const quickStatus = $("advanceQuickStatus");
      ...
      if (count) count.classList.toggle("advance-preview-hidden", !selectedAvailableDates.length);
      ...
      if (quickStatus) quickStatus.textContent = labels.length ? `${labels.length}日分` : "未選択";
      if (!box) return;
      box.classList.toggle("advance-preview-hidden", !selectedAvailableDates.length);
      if (!selectedAvailableDates.length) {
        box.textContent = "申請できる稼働日がありません";
        return;
      }
      box.innerHTML = selectedAvailableDates.map((date) => `<span>${formatShortDate(date)}(${dayOfWeekLabel(date)})</span>`).join("");
    }
    ...
    function updateSummary() {
      ...
      $("advanceTransferMeta").textContent = workedDays
        ? `${workedDays}日分を選択中。金額を確認して、右へスワイプしてください。`
        : "退勤済みの稼働日を選ぶと、振込予定額を自動計算します。";
      if ($("advanceQuickTransfer")) $("advanceQuickTransfer").textContent = workedDays ? formatMoney(transferAmount) : "選択後";
      ...
      const summaryCard = document.querySelector(".advance-preview-summary-card");
      if (summaryCard) {
        summaryCard.classList.toggle("is-empty", !workedDays);
        summaryCard.classList.toggle("is-ready", Boolean(workedDays && amount && !hasOverlap));
      }
      document.body.classList.toggle("has-advance-selection", Boolean(workedDays));
      syncSubmitReady();
    }
  ...
  function initAdmin() {
    ...
    function startAdminAutoRefresh() {
      ...
      autoRefreshTimer = window.setInterval(refreshDashboardQuietly, 90000);
    }
