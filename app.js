  ...
  async function requireDriverLogin(requestedDriverId) {
    ...
    if (session && session.driver && session.token && (!requestedDriverId || session.driver.id === requestedDriverId)) {
      ...
      if (config.API_BASE_URL) {
        ...
        if (profile && profile.userId && session.driver.lineUserId !== profile.userId) {
          try {
            const linked = await apiPost({
              type: "driver_line_link",
              driverToken: session.token,
              driverId: session.driver.id,
              lineUserId: profile.userId,
              lineDisplayName: profile.displayName || ""
            }, { skipAdminToken: true });
            if (linked && linked.driver) writeDriverSession(linked.driver, session.token);
          } catch (error) {
