(function () {
    ...
    if (config.API_BASE_URL) {
      ...
      try {
        ...
        if (remote && remote.found && remote.row) {
          ...
          existing = null;
          writeStore("attendance", rows.filter((row) => !(row.driverId === driver.id && sameDate(row.date, workDate))));
        }
      } catch (error) {
        existing = null;
        clearDriverAttendanceCache(driver.id, workDate);
  ...
  function buildDashboard(month) {
    ...
    const attendance = readStore("attendance", []).map(normalizeAttendanceRow).filter((row) => inMonth(row.date, month));
    const advances = readStore("advance", []).map(normalizeAdvanceRow).filter((row) => inMonth(row.date, month));
    ...
    const todayRows = readStore("attendance", []).map(normalizeAttendanceRow).filter((row) => sameDate(row.date, today));
  ...
  function initAdmin() {
    ...
    function render() {
      ...
      $("todayList").innerHTML = data.drivers.map((driver) => {
        ...
        const status = row ? attendanceStatusText(row.status) : "未出勤";
        return item(driver.name, `${driver.siteName} ・ ${driver.contractType || ""}`, [{ label: status, kind: row && isWorkingStatus(row.status) ? "warn" : "good" }], `<button class="button ghost small" data-driver="${driver.id}">詳細</button>`);
      }).join("");
